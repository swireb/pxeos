...
fsUUIDFileName() {
    ...
    [[ -z $imagePath ]] && handleError "No image path passed (${FUNCNAME[0]})\n   Args Passed: $*"
    [[ -z $disk_number ]] && handleError "No drive number passed (${FUNCNAME[0]})\n   Args Passed: $*"
    fsuuidfilename="$imagePath/d${disk_number}.original.fsuuids"
}

# Save filesystem UUID (for /dev/disk/by-uuid matching).
# File format: "<part_number> <fstype> <uuid>"
#
# $1 = file path
# $2 = partition device (e.g. /dev/sda2)
saveFsUUID() {
    local file="$1"
    local part="$2"
    [[ -z $file ]] && handleError "No file to receive from passed (${FUNCNAME[0]})\n   Args Passed: $*"
    ...
    # Extended partitions have no filesystem UUID
    local parttype=""
    getPartType "$part"
    [[ $parttype == +(0x5|0xf) ]] && return

    local uuid=""
    uuid="$(blkid -s UUID -o value "$part" 2>/dev/null | head -n 1)"
    [[ -z $uuid ]] && return

    local part_number=""
    getPartitionNumber "$part"
    [[ -z $part_number ]] && return

    echo " * Saving filesystem UUID ($uuid) for ($part)"
    echo "${part_number} ${fstype} ${uuid}" >> "$file"
}

# Restore filesystem UUIDs using saved file.
# Supported: ext* (tune2fs), xfs (xfs_admin), btrfs (btrfstune), swap is handled separately.
#
# $1 = file path
# $2 = partition device
restoreFsUUID() {
    ...
    local line=""
    line="$(awk "/(^${part_number} )/{print \$0; exit}" "$file")"
    [[ -z $line ]] && return

    local fstype uuid
    fstype="$(echo "$line" | awk '{print $2}')"
    uuid="$(echo "$line" | awk '{print $3}')"
    [[ -z $fstype || -z $uuid ]] && return
    [[ $fstype == swap ]] && return

    # Only attempt when tool exists; otherwise leave as-is.
    case "$fstype" in
        ext2|ext3|ext4|extfs)
            command -v tune2fs >/dev/null 2>&1 || return
            dots "Restoring filesystem UUID ($fstype)"
            tune2fs -U "$uuid" "$part" >/dev/null 2>&1
            ;;
        xfs)
            command -v xfs_admin >/dev/null 2>&1 || return
            dots "Restoring filesystem UUID ($fstype)"
            xfs_admin -U "$uuid" "$part" >/dev/null 2>&1
            ;;
        btrfs)
            command -v btrfstune >/dev/null 2>&1 || return
            dots "Restoring filesystem UUID ($fstype)"
            btrfstune -U "$uuid" "$part" >/dev/null 2>&1
            ;;
        *)
            return
            ;;
...
restoreAllFsUUIDs() {
    ...
    [[ -r "$fsuuidfilename" ]] || return

    local parts="" part="" part_number=""
    getPartitions "$disk"
    for part in $parts; do
        getPartitionNumber "$part"
        [[ $imgPartitionType != all && $imgPartitionType -ne $part_number ]] && continue
        restoreFsUUID "$fsuuidfilename" "$part"
    done
}
